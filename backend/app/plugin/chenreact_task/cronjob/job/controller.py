...
@JobRouter.get(
    ...
    dependencies=[Depends(AuthPermission(["chenreact_task:cronjob:job:query"]))],
)
...
@JobRouter.post(
    ...
    dependencies=[Depends(AuthPermission(["chenreact_task:cronjob:job:scheduler"]))],
)
...
@JobRouter.delete(
    ...
    response_model=ResponseSchema[None],
    dependencies=[Depends(AuthPermission(["chenreact_task:cronjob:job:task"]))],
)
...
@JobRouter.post(
    ...
    dependencies=[Depends(AuthPermission(["chenreact_task:cronjob:job:update"]))],
)
...
@JobRouter.delete(
    ...
    response_model=ResponseSchema[None],
    dependencies=[Depends(AuthPermission(["chenreact_task:cronjob:job:delete"]))],
)
...
async def get_job_log_list_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:cronjob:job:query"]))],
) -> JSONResponse:
    """
    查询执行日志列表
...
async def get_job_log_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:cronjob:job:detail"]))],
) -> JSONResponse:
    """
    获取执行日志详情
...
async def delete_job_log_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:cronjob:job:delete"]))],
) -> JSONResponse:
    """
    删除执行日志
