...
async def get_workflow_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:workflow:definition:detail"]))],
) -> JSONResponse:
    """
...
async def get_workflow_list_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:workflow:definition:query"]))],
) -> JSONResponse:
    """
    分页查询工作流列表。
...
async def create_workflow_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:workflow:definition:create"]))],
) -> JSONResponse:
    """
    创建草稿工作流。
...
async def update_workflow_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:workflow:definition:update"]))],
) -> JSONResponse:
    """
    更新工作流及画布。
...
async def delete_workflow_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:workflow:definition:delete"]))],
) -> JSONResponse:
    """
    批量删除工作流。
...
async def execute_workflow_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_task:workflow:definition:execute"]))],
) -> JSONResponse:
    """
