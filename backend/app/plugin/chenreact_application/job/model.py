...
class AppJobModel(ModelMixin, UserMixin):
    """
    定时任务调度表
    - 0: 运行中
    - 1: 暂停中
    ...
    # 关联关系
    job_logs: Mapped[list["AppJobLogModel"] | None] = relationship(
        back_populates="job", lazy="selectin"
    )


class AppJobLogModel(ModelMixin):
    """
    定时任务调度日志表
    """
    ...
    job: Mapped["AppJobModel | None"] = relationship(back_populates="job_logs", lazy="selectin")
