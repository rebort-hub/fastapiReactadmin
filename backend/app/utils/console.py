...
def console_run(
    ...
    *,
    ...
    # 核心服务信息
    service_info = Text()
    service_info.append(f"服务名称 {settings.TITLE} • 极速体验开发", style="bold magenta")
    ...
    service_info.append(
        f"\n重载配置: {'启动成功' if reload else '❌ 关闭'}",
        style="bold italic",
    )
    service_info.append(
        f"\n调试模式: {'启动成功' if settings.DEBUG else '❌ 关闭'}",
        style="bold italic",
    )
    service_info.append(
        f"\n{settings.DATABASE_TYPE}: {'启动成功' if database_ready else '❌ 关闭'}",
        style="bold italic",
    )
    service_info.append(
        f"\nRedis: {'启动成功' if redis_ready else '❌ 关闭'}",
        style="bold italic",
    )
    service_info.append(
        f"\n调度器: {'启动成功' if scheduler_ready else '❌ 关闭'}",
        style="bold italic",
    )
    service_info.append(
        f"\n限流器: {'启动成功' if limiter_ready else '❌ 关闭'}",
        style="bold italic",
    )
...
def console_close() -> None:
    ...
    shutdown_content.append("FastapireactAdmin 服务关闭")
    shutdown_content.append(f"\n⏰ {datetime.now().strftime('%H:%M:%S')}")
    shutdown_content.append("\nReact后台已经关闭，欢迎使用后台服务！下次再见", style="dim")
