...
PLUGIN_MODULE_PREFIX = "chenreact_"
...
def get_dynamic_router() -> APIRouter:
    ...
    try:
        ...
        controller_files = list(base_dir.glob(f"{PLUGIN_MODULE_PREFIX}*/**/controller.py"))
        ...
        for file in controller_files:
            ...
            # 生成路由前缀 (chenreact_xxx -> /xxx)
            suffix = (
                top_module[len(PLUGIN_MODULE_PREFIX) :]
                if top_module.startswith(PLUGIN_MODULE_PREFIX)
                else ""
            )
            if not suffix:
                log.error(
                    f"❌ 跳过异常顶级目录名（须为 {PLUGIN_MODULE_PREFIX} 前缀且后面还有名称）: {top_module!r}，"
                    f"文件: {file}"
                )
                continue
            prefix = f"/{suffix}"
        ...
        for prefix, container_router in sorted(container_routers.items()):
            ...
            if route_count == 0:
                log.warning(
                    f"⚠️ 容器前缀 {prefix} 下未挂载任何子路由（可能该插件下所有 controller 均未导出 APIRouter）"
                )
            log.info(f"自动发现注册容器（热插拔）: {prefix} (子路由数: {route_count})")

        log.info(f"自动识别动态路由发现完成: 共 {len(container_routers)} 个容器前缀")
        return root_router
