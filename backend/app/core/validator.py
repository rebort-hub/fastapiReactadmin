...
USERNAME_EMPTY_MSG = "账号不能为空"
USERNAME_FORMAT_MSG = "账号需字母开头，3-32位，仅含字母/数字/_ . -"
PASSWORD_MIN_LEN = 6
PASSWORD_MAX_LEN = 128
PASSWORD_MIN_MSG = "密码长度至少6位"
PASSWORD_MAX_MSG = "密码长度不能超过128个字符"


def username_validator(value: str) -> str:
    """
    账号格式验证器（字母开头，3-32 位，仅含字母/数字/_ . -）。
    ...
    v = value.strip()
    if not v:
        raise ValueError(USERNAME_EMPTY_MSG)
    if not USERNAME_PATTERN.match(v):
        raise ValueError(USERNAME_FORMAT_MSG)
    return v


def password_plain_validator(value: str) -> str:
    """
    明文密码长度验证器（6-128 位）。
    ...
    if len(value) < PASSWORD_MIN_LEN:
        raise ValueError(PASSWORD_MIN_MSG)
    if len(value) > PASSWORD_MAX_LEN:
        raise ValueError(PASSWORD_MAX_MSG)
    return value
