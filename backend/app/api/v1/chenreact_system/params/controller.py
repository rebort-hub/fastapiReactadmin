...
async def get_type_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:param:detail"]))],
) -> JSONResponse:
    """
    获取参数详情
...
async def get_obj_by_key_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:param:query"]))],
) -> JSONResponse:
    """
    根据配置键获取参数详情
...
async def create_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:param:create"]))],
) -> JSONResponse:
    """
    创建参数
...
async def update_objs_controller(
    ...
    id: Annotated[int, Path(description="参数ID")],
    redis: Annotated[Redis, Depends(redis_getter)],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:param:update"]))],
) -> JSONResponse:
    """
    修改参数
...
async def delete_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:param:delete"]))],
) -> JSONResponse:
    """
    删除参数
...
async def export_obj_list_controller(
    search: Annotated[ParamsQueryParam, Depends()],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:param:export"]))],
) -> StreamingResponse:
    """
    导出参数
...
@ParamsRouter.post(
    "/upload",
    summary="上传文件",
    dependencies=[Depends(AuthPermission(["chenreact_system:param:upload"]))],
    response_model=ResponseSchema[None],
)
