...
from app.api.v1.chenreact_monitor.online.schema import OnlineOutSchema
from app.api.v1.chenreact_system.user.crud import UserCRUD
from app.api.v1.chenreact_system.user.model import UserModel
...
class CaptchaService:
    ...
    async def get_captcha_service(cls, redis: Redis) -> dict[str, CaptchaKey | CaptchaBase64]:
        ...
        if not settings.CAPTCHA_ENABLE:
            return CaptchaOutSchema(enable=False).model_dump()
...
class AutoLoginService:
    ...
    # 免登录Token前缀
    AUTO_LOGIN_PREFIX = "fastapireactadmin:auto_login:"
    # Token有效期(秒) - 5分钟
    TOKEN_EXPIRE = 300
