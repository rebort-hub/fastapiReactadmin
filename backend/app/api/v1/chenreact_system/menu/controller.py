...
async def get_menu_tree_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:menu:query"]))],
) -> JSONResponse:
    """
    查询菜单树。
...
async def get_obj_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:menu:detail"]))],
) -> JSONResponse:
    """
    查询菜单详情。
...
async def create_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:menu:create"]))],
) -> JSONResponse:
    """
    创建菜单。
...
async def update_obj_controller(
    ...
    id: Annotated[int, Path(description="菜单ID")],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:menu:update"]))],
) -> JSONResponse:
    """
    修改菜单。
...
async def delete_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:menu:delete"]))],
) -> JSONResponse:
    """
    删除菜单。
...
async def batch_set_available_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:menu:patch"]))],
) -> JSONResponse:
    """
    批量修改菜单状态。
