...
async def get_obj_list_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:log:query"]))],
) -> JSONResponse:
    """
    查询日志
...
async def get_obj_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:log:detail"]))],
) -> JSONResponse:
    """
    获取日志详情
...
async def delete_obj_log_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:log:delete"]))],
) -> JSONResponse:
    """
    删除日志
...
async def export_obj_list_controller(
    search: Annotated[OperationLogQueryParam, Depends()],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:log:export"]))],
) -> StreamingResponse:
    """
    导出日志
