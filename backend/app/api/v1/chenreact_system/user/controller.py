...
async def get_obj_list_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:query"]))],
) -> JSONResponse:
    """
    查询用户
...
async def get_obj_detail_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:detail"]))],
) -> JSONResponse:
    """
    查询用户详情
...
async def create_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:create"]))],
) -> JSONResponse:
    """
    创建用户
...
async def update_obj_controller(
    ...
    id: Annotated[int, Path(description="用户ID")],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:update"]))],
) -> JSONResponse:
    """
    修改用户
...
async def delete_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:delete"]))],
) -> JSONResponse:
    """
    删除用户
...
async def batch_set_available_obj_controller(
    ...
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:patch"]))],
) -> JSONResponse:
    """
    批量修改用户状态
...
@UserRouter.post(
    ...
    response_model=ResponseSchema[None],
    dependencies=[Depends(AuthPermission(["chenreact_system:user:download"]))],
)
...
async def export_obj_list_controller(
    page: Annotated[PaginationQueryParam, Depends()],
    search: Annotated[UserQueryParam, Depends()],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:export"]))],
) -> StreamingResponse:
    """
    导出用户
...
async def import_obj_list_controller(
    file: UploadFile,
    auth: Annotated[AuthSchema, Depends(AuthPermission(["chenreact_system:user:import"]))],
) -> JSONResponse:
    """
    导入用户
