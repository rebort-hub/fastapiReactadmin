# fastapiReactadmin - Backend

基于 FastAPI+React19 框架构建的企业级后端架构，为前端 React19 管理系统提供完整的 API 服务支持。
...
│   ├── api/                 # API 接口模块
│   │   └── v1/              # API v1 版本
│   │       ├── chenreact_system/   # 系统管理模块
│   │       ├── chenreact_monitor/  # 系统监控模块
│   │       └── chenreact_*/       # 其他业务模块
│   ├── common/              # 公共组件（常量、枚举、响应封装）
│   ├── config/              # 项目配置文件
│   ├── core/                # 核心模块（数据库、中间件、安全）
│   ├── chenreact_task/         # 定时任务模块
│   ├── plugin/              # 插件模块（二开目录）
│   ├── scripts/             # 初始化脚本和数据
│   └── utils/               # 工具类（验证码、文件上传等）
├── env/                     # 环境配置文件
...
每个业务模块采用统一的分层结构：注意：（二次开发统一在APP目录下的plugin创建新模块，名称按照严格按照如下要求执行，文件目录，子文件，例如开发UI自动化模块，chenreact_ui）
...
- **Python**: 3.10+
- **数据库**: MySQL 8.0+ / PostgreSQL 13+ （连接串在 `env/.env.dev`）
- **Redis**: 与 `.env.dev` 中配置一致

### 第一次在本机跑起来

1. 复制 `env/.env.dev.example` → `env/.env.dev`，填写数据库、Redis 等（先在 DB 中建好空库）。 
2. 接口文档示例：`http://127.0.0.1:8001/docs`（端口见 `.env.dev` 中 `SERVER_PORT`）。
