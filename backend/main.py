...
fastapireactadmin_cli = typer.Typer()
...
@fastapireactadmin_cli.command(
    name="run",
    help="启动 FastapireactAdmin 服务, 运行 uv run main.py run --env=dev 不加参数默认 dev 环境",
)
def run(
    env: Annotated[
...
@fastapireactadmin_cli.command(
    name="revision",
    help="生成新的 Alembic 迁移脚本, 运行 uv run main.py revision --env=dev",
)
def revision(
    env: Annotated[
        EnvironmentEnum, typer.Option("--env", help="运行环境 (dev, prod)")
...
@fastapireactadmin_cli.command(
    name="upgrade",
    help="应用最新的 Alembic 迁移, 运行 uv run main.py upgrade --env=dev",
)
def upgrade(
    env: Annotated[
        EnvironmentEnum, typer.Option("--env", help="运行环境 (dev, prod)")
