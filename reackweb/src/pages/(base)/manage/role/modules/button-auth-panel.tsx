...
const BUTTON_TYPE: Api.SystemModule.MenuType = 3;
const NON_BUTTON_TYPES: Api.SystemModule.MenuType[] = [1, 2, 4];
...
function convertMenuTreeToDataNode(menus: Api.SystemModule.Menu[]): DataNode[] {
  return menus.map(menu => ({
    ...
    key: menu.id,
    title: menu.permission ? `${menu.name} (${menu.permission})` : menu.name
  }));
}
...
const ButtonAuthPanel: FC<Props> = memo(({ onSaved, roleId, showFooter = true }) => {
  ...
  const treeData = useMemo(() => {
    return convertMenuTreeToDataNode(filterMenuTreeByTypes(menuTree, [BUTTON_TYPE]));
  }, [menuTree]);
  ...
  async function init() {
    ...
    try {
      ...
      setChecks(collectMenuIdsByTypes(detail.menus ?? [], [BUTTON_TYPE]));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const otherMenuIds = collectMenuIdsByTypes(roleSnapshot.menus ?? [], NON_BUTTON_TYPES);

      await fetchSetRolePermission({
        data_scope: roleSnapshot.data_scope ?? 1,
        dept_ids: roleSnapshot.depts?.map(item => item.id) ?? [],
        menu_ids: [...new Set([...otherMenuIds, ...(checks ?? [])])],
        role_ids: [roleId]
      });
...
export default ButtonAuthPanel;
