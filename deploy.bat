...
echo   🌐 前端 reackweb    - React 前端 (端口: 9527)
echo.
...
call :start_service "backend"   "%SCRIPT_DIR%backend"             "uv run main.py run --env=dev"    "fastapiReactadmin.Backend"  "%BACKEND_PID%"
call :start_service "reackweb"  "%SCRIPT_DIR%reackweb"        "pnpm dev"                         "fastapiReactadmin.reackweb"      "%WEB_PID%"
...
echo   🌐 前端 Web: http://localhost:9527
echo.
goto :end
